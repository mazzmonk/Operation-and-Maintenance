...
# author: jack
# date: 20190618
# setup kernel, env, firewall, username/password etc.
##############################################
set -e
set -x
...
userdel ${USERNAME} -r
groupadd ${USERNAME} -g 101 
useradd -u 101 -g ${USERNAME} ${USERNAME}
mkdir -p ${HOME}/bin 
mkdir -p ${HOME}/logs/{nginx,apache}
mkdir -p ${HOME}/src 
mkdir -p ${HOME}/www
mkdir -p ${HOME}/backup
mkdir -p ${HOME}/tmp
...
echo "${USERNAME}:${USERNAME}123"  > /root/pass.list
chpasswd aviva < /root/pass.list
